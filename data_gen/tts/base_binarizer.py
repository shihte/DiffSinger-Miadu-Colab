...
class BaseBinarizer:
    ...
    def process_data(self, prefix):
        ...
        if num_workers > 1:
            it = zip(tqdm(meta_data), chunked_multiprocess_run(self.process_item, args, num_workers=num_workers))
        else:
            # Serial processing to avoid hparams issues in multiprocessing
            it = zip(tqdm(meta_data), (self.process_item(*arg) for arg in args))

        for f_id, (_, item) in enumerate(it):
            if item is None:
                continue
